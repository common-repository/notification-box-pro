...
/*
/**
 * Plugin Name: Notification Box
 * Plugin URI: https://zotabox.com/zbv2/notificationbox?utm_source=wordpress.com&utm_medium=Notification%20Box&utm_campaign=ecommerce%20plugins&authuser=anonymous
 * Description: Notification Box for Wordpress. Fade in boxes to promote your products on your website.
 * Version: 1.1.5
 * Author: Zotabox
 * Author URI: https://zotabox.com/dashboard/?utm_source=wordpress.com&utm_medium=Notification Box&utm_campaign=ecommerce%20plugins&authuser=anonymous
 * License: SMB 1.0
 */

//Add some free tools

add_action( 'admin_init', 'zb_nb_admin_init' );
function zb_nb_admin_init(){
	/* Register stylesheet. */
	wp_register_style( 'css_main', plugins_url('assets/css/style.css', __FILE__) );
	wp_enqueue_style('css_main');
    /* Register js. */
	wp_register_script( 'zb_nb_admin_init', plugins_url('assets/js/main.js?v='.time(), __FILE__) );
	wp_enqueue_script('zb_nb_admin_init');
...
register_deactivation_hook( __FILE__, 'zb_nb_deactivate' );
function zb_nb_deactivate(){
	update_option( 'ztb_status_message', 2 );
	update_option( 'ztb_status_disconnect', 1 );
}

register_activation_hook( __FILE__, 'zb_nb_activate' );
function zb_nb_activate() {
	update_option( 'ztb_status_message', 1 );
}

add_action('admin_notices', 'zb_nb_show_admin_messages');
function zb_nb_show_admin_messages()
{
	...
	$message_intall = '<div id="message" class="updated fade">
		    ...
		    <a href="/wp-admin/admin.php?page=zb_nb">
	...
	if ( is_plugin_active( 'notification-box/notification-box.php') ) {
...
add_filter( 'plugin_action_links_' . plugin_basename(__FILE__), 'zb_nb_add_action_links' );
function zb_nb_add_action_links ( $links ) {
	 $mylinks = array(
	 '<a href="' . admin_url( 'admin.php?page=zb_nb' ) . '">Settings</a>',
	 );
	return array_merge( $links, $mylinks );
}

add_action('admin_menu', 'zb_nb_admin_menu');
function zb_nb_admin_menu() {
	add_menu_page('Notification Box', 'Notification Box', 'administrator', 'zb_nb', 'zb_nb_setting',plugins_url( 'zotabox.png', __FILE__ ));
}

function zb_nb_setting(){
	$domain_action = 'https://zotabox.com';
	$token_key = wp_create_nonce('update_zb_nb_code');
	...
	if(isset($access_key) && !empty($access_key) && strlen($access_key) > 0 && $ztb_status_disconnect == $connected){
		...
		$button = '<a  target="zotabox" href="'.$domain_action.'/customer/access/PluginLogin/?customer='.$ztb_id.'&access='.$access_key.'&domain_secure_id='.$domain.'&app=nb&platform=wordpress&utm_source=wordpress.com">
			Configure your tools
		</a>';
		$form = '';
	}else{
		$form = '<form class="ztb-register-form" target="_blank" method="POST" action="'.$domain_action.'/customer/access/PluginAuth?app=nb&utm_source=wordpress.com&utm_medium=Notification Box&utm_campaign=ecommerce%20plugins&token='.$token_key.'&platform=wordpress&access='.$access_key.'" id="account-info">
					<div class="form-group">
						...
						<input type="hidden" name="utm_medium" value="Notification Box" />
					...
					<div class="form-group button-wrapper">
						<input zb-plugin="zb_nb" class="ztb-submit-button" type="submit" value="Start Using Your New Tools Now" /><br><br>
	...
	<div class="ztb-wrapper">
		<div class="ztb-logo">
			<a href="https://zotabox.com/dashboard/?utm_source=wordpress.com&utm_medium=Notification Box&utm_campaign=ecommerce%20plugins&authuser=anonymous" title="Zotabox" target="zotabox">
...
function insert_zb_nb_code(){
	if(!is_admin()){
		...
		if(!empty($domain) && strlen($domain) > 0 && $ztb_status_disconnect == $connected){
			print_r(html_entity_decode(print_zb_nb_code($domain)));
		}
	}
}
add_action( 'wp_head', 'insert_zb_nb_code' );


add_action("wp_ajax_update_zb_nb_code", "update_zb_nb_code");
add_action("wp_ajax_nopriv_update_zb_nb_code", "update_zb_nb_code");

function update_zb_nb_code(){
	if(isset($_REQUEST['token']) && wp_verify_nonce($_REQUEST['token'], 'update_zb_nb_code')){
		...
		if(!isset($domain) || empty($domain)){
			$redirect = admin_url('admin.php?page=zb_nb');
        	wp_safe_redirect($redirect);
		}else{
			if(wp_verify_nonce($_REQUEST['token'], 'update_zb_nb_code')){
...
function print_zb_nb_code($domainSecureID = "", $isHtml = false) {
